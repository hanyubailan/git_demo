...
static void (*uart1_buf_decode)(volatile uint8_t uart1_rx_buf[UART_RX_BUF_LEN]) = NULL;   //uartX_buf_decode 是回调函数指针，用于用户自定义的数据解码逻辑，当 DMA 接收完成后，调用对应的解码函数处理接收的数据。
...
{
    __HAL_UART_CLEAR_IDLEFLAG(huart);          //先清除空闲中断标志位，防止开启中断时立马进入中断
    __HAL_UART_ENABLE_IT(huart, UART_IT_IDLE); //使能空闲中断

    SET_BIT(huart->Instance->CR3, USART_CR3_DMAR); //将串口对应的DMA打开  串口控制寄存器3的接受给打开
    if (huart->Instance == USART1)
    {
        DMAEx_MultiBufferStart_NoIT(huart->hdmarx,                   //指向接收通道
                                    (uint32_t)&huart->Instance->DR,  //从UART 的 DR 寄存器读取接收的数据
                                    (uint32_t)uart1_rx_buf[0],
                                    (uint32_t)uart1_rx_buf[1],
                                    UART_RX_BUF_LEN);                //单个缓冲区的大小
