...
int32_t PID_calc(pid_type_def *pid, int32_t ref, int32_t set)
{
    ...
    pid->fdb = ref;
    pid->error[0] = set - ref;
    if (pid->mode == PID_POSITION)                            // 满足最初的公式pid_out = Kp * E + Ki * (E累和) + Kd * (E1 - E0)
    {
    ...
    else if (pid->mode == PID_DELTA)
    {
        ...
        pid->Dbuf[2] = pid->Dbuf[1];
        pid->Dbuf[1] = pid->Dbuf[0];
        pid->Dbuf[0] = (pid->error[0] - 2.0f * pid->error[1] + pid->error[2]);   //等于（E1-E2）-（E2-E3）
        pid->Dout = pid->Kd * pid->Dbuf[0];
        pid->out += pid->Pout + pid->Iout + pid->Dout;
...
/**
  * @brief          模式1下的绝对值的加减计算和过零处理
  */
int32_t MODE1_ChangeRPM(int32_t set, float ChangeRPM)
{
    int sign = (set < 0) ? -1 : 1;  // 获取原始数的符号
    int32_t abs_set = abs(set);       // 获取原始数的绝对值
    
    // 做出变化（加或减），并且对结果进行限制
    int32_t modified_abs_value = abs_set + ChangeRPM;

    // 如果加法后绝对值超过 3000，限制为 3000
...
int32_t chang_sign(int32_t HubMotor)    // 翻转电机
{
	return HubMotor*(-1);
};
